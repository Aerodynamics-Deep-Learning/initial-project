"""
We DO NOT claim the ownership of the code below, it is a direct copy from the GitHub repository:
https://github.com/pswpswpsw/nif/blob/main/nif/model.py
...
class NIF(object):
    ...
    def _call_shape_net(input_s, pnet_output, si_dim, so_dim, n_sx, l_sx, activation, variable_dtype):
        ...
        w_hidden_list = []
        for i in range(l_sx):
            w_tmp = tf.reshape(pnet_output[:, si_dim * n_sx + i * n_sx**2 : si_dim * n_sx + (i + 1) * n_sx**2,],[-1, n_sx, n_sx],name="w_hidden_snet_{}".format(i),)
            w_hidden_list.append(w_tmp)
        w_l = tf.reshape(pnet_output[:,si_dim * n_sx+ l_sx * n_sx**2 : si_dim * n_sx+ l_sx * n_sx**2+ so_dim * n_sx,],[-1, n_sx, so_dim],name="w_last_snet",)
        ...
        b_hidden_list = []
        for i in range(l_sx):
            b_tmp = tf.reshape(pnet_output[:, n_weights + n_sx + i * n_sx : n_weights + n_sx + (i + 1) * n_sx],[-1, n_sx],name="b_hidden_snet_{}".format(i),)
